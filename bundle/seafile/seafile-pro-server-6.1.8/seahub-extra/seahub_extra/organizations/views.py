...
from seahub_extra.organizations.settings import RESERVED_SUBDOMAINS, \
    ORG_AUTO_URL_PREFIX, ORG_MEMBER_QUOTA_ENABLED
...
def get_org_users_by_url_prefix(url_prefix, start, limit):
    return seaserv.ccnet_threaded_rpc.get_org_emailusers(url_prefix,
                                                          start, limit)
...
def org_add(request):
    ...
    if form.is_valid():
        ...
        try:
            ...
            logger.error(e)
            err_msg = _(u'Fail to create organization owner %s.') % email
    ...
    if request.method == 'POST':
        ...
        if ORG_AUTO_URL_PREFIX:
            ...
            if url_prefix is None:
                messages.error(request, _("Failed to create organization account, please try again later."))
                return render_to_response('organizations/org_register.html', {
                    'form': form,
...
def org_user_admin(request):
    ...
    return render_to_response('organizations/org_user_admin.html', {
            'users': users,
...
def org_useradmin_admins(request):
    ...
    admin_users = []
    not_admin_users = []
    users = get_org_users_by_url_prefix(org.url_prefix, -1, -1)
    last_logins = UserLastLogin.objects.filter(username__in=[x.email for x in users])
    ...
    admin_users.insert(0, self)

    return render_to_response('organizations/org_useradmin_admins.html', {
            'users': admin_users,
...
def org_user_add(request):
    ...
    org_members = len(get_org_users_by_url_prefix(url_prefix, -1, -1))
    ...
    if ORG_MEMBER_QUOTA_ENABLED:
        ...
        if org_members_quota is not None and org_members >= org_members_quota:
            result['error'] = _(u'Failed. You can only invite %d members.') % org_members_quota
...
def org_user_search(request):
    ...
    org = request.user.org
    org_users = get_org_users_by_url_prefix(org.url_prefix, -1, -1)

    results = []
    if search_patt:
        for u in org_users:
            if search_patt in u.email:
                results.append(u)
    ...
    return render_to_response('organizations/user_search.html', {
            'users': results,
            'email': search_patt,
            }, context_instance=RequestContext(request))
...
def org_user_remove(request, user_id):
    ...
    if not org_user_exists(org.org_id, user.username):
        messages.error(request, _(u'Failed to delete: the user does not belong to the organization.'))
        return HttpResponseRedirect(next)
...
def org_user_reset(request, user_id):
    ...
    try:
        ...
        messages.error(request, _(u'Failed to reset password: the user does not exist'))
        return HttpResponseRedirect(next)

    org = request.user.org
    if not org_user_exists(org.org_id, user.username):
        messages.error(request, _(u'Failed to reset password: the user does not belong to the organization.'))
        return HttpResponseRedirect(next)
...
def org_user_make_admin(request, user_id):
    ...
    org = request.user.org
    if not org_user_exists(org.org_id, user.username):
        messages.error(request, _(u'Failed to set admin: the user does not belong to the organization.'))
        return HttpResponseRedirect(next)
...
def org_user_remove_admin(request, user_id):
    ...
    org = request.user.org
    if not org_user_exists(org.org_id, user.username):
        messages.error(request, _(u'Failed to revoke admin: the user does not belong to the organization.'))
        return HttpResponseRedirect(next)
...
def org_repo_admin(request):
    ...
    return render_to_response('organizations/org_repo_admin.html', {
            'repos': repos,
            'current_page': current_page,
            'prev_page': current_page-1,
            'next_page': current_page+1,
...
def org_repo_transfer(request):
    ...
    if not org_user_exists(org_id, new_owner) or \
        ...
        messages.error(request, _(u'Failed to transfer, user does not exist.'))
        return HttpResponseRedirect(next)

    if not is_org_repo(org_id, repo_id):
        messages.error(request, _(u'Failed to transfer, library does not exist.'))
        return HttpResponseRedirect(next)
...
def org_repo_delete(request, repo_id):
    ...
    org_id = request.user.org.org_id
    if not is_org_repo(org_id, repo_id):
        messages.error(request, _(u'Failed to delete, library does not exist.'))
        return HttpResponseRedirect(next)
...
def org_repo_search(request):
    ...
    return render_to_response('organizations/org_repo_search.html', {
            'repos': repos,
            'name': repo_name,
            'owner': owner,
            }, context_instance=RequestContext(request))
...
def org_user_info(request, email):
    ...
    return render_to_response('organizations/userinfo.html', {
...
def org_user_set_quota(request, email):
    ...
    if not is_valid_username(email):
        result['error'] = _(u'Failed to set quota: invalid email')
...
def org_publink_admin(request):
    ...
    return render_to_response('organizations/org_publink_admin.html', {
            'publinks': publinks,
            'current_page': current_page,
            'prev_page': current_page-1,
            'next_page': current_page+1,
...
def org_manage(request):
    ...
    try:
        org_members = get_org_users_by_url_prefix(url_prefix, -1, -1)
    except Exception as e:
        logger.error(e)
        org_members = []
    ...
    if org_members:
        ...
        for m in org_members:
            storage_usage += get_org_user_self_usage(org_id, m.email)
    ...
    return render_to_response('organizations/org_manage.html', {
...
def org_log_file_audit(request):
    ...
    return render_to_response('organizations/org_file_audit.html', {
            'events': events,
...
def org_log_file_update(request):
    ...
    return render_to_response('organizations/org_file_update.html', {
            'events': events,
            'user_selected': user_selected,
            'repo_selected': repo_selected,
            'extra_href': extra_href,
...
def org_log_perm_audit(request):
    ...
    return render_to_response('organizations/org_perm_audit.html', {
            'events': events,
            'user_selected': user_selected,
            'repo_selected': repo_selected,
            'extra_href': extra_href,
...
def org_admin_group_info(request, group_id):
    ...
    return render_to_response('organizations/org_admin_group_info.html', {
            'group': group,
            'repos': repos,
            'members': members,
            }, context_instance=RequestContext(request))
