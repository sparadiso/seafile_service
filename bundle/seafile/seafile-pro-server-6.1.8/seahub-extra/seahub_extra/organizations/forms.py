...
class OrgRegistrationForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not is_valid_username(email):
            raise forms.ValidationError(_("Email is not valid."))
    ...
    def clean_url_prefix(self):
        ...
        if slug_re.match(url_prefix):
            if seaserv.ccnet_threaded_rpc.get_org_by_url_prefix(url_prefix):
                raise forms.ValidationError(_("Someone already has that prefix. Try another?"))
            else:
                return url_prefix
        else:
            raise forms.ValidationError(_("URL prefix can only be letters(a-z), numbers, and the underscore character."))
