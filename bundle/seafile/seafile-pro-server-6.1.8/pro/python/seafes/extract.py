...
class Extractor(object):
    def __init__(self, func, text_size_limit):
        self.func = func
...
class ExtractorFactory(object):
    ...
    def get_extractor(cls, filename):
        ...
        if not func:
            return None
        return Extractor(func, cls.get_text_size_limit(filename))
    ...
    @classmethod
    def get_text_size_limit(cls, filename):
        if is_text_file(filename):
            limit = seafes_config.text_size_limit
        elif is_office_pdf(filename):
            limit = seafes_config.office_size_limit
        else:
            limit = -1
        return limit
